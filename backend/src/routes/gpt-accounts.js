...
const normalizeCapacity = (value, fallback = DEFAULT_MAX_CAPACITY) => {
...
const ACCOUNT_SELECT_FIELDS = `
  ...
  user_count,
  invite_count,
  COALESCE(max_capacity, ${DEFAULT_MAX_CAPACITY}) AS max_capacity,
...
const mapAccountRow = (row) => ({
  ...
  userCount: row[4],
  inviteCount: row[5],
  maxCapacity: normalizeCapacity(row[6]),
  reservedSlots: Number(row[7] || 0),
  chatgptAccountId: row[8],
  oaiDeviceId: row[9],
  expireAt: row[10] || null,
  isOpen: Boolean(row[11]),
  isDemoted: false,
...
router.get('/', async (req, res) => {
  try {
    ...
    const total = countResult[0]?.values?.[0]?.[0] || 0
	    ...
	    const accounts = (dataResult[0]?.values || []).map(mapAccountRow)
...
router.post('/', async (req, res) => {
  try {
    ...
    const maxCapacity = normalizeCapacity(body.maxCapacity ?? body.max_capacity)
    ...
    // 设置默认人数为1而不是0
    const finalUserCount = userCount !== undefined ? userCount : 0
    const generatedClientProfile = generateAccountClientProfile(normalizedEmail, normalizedOaiDeviceId)

    db.run(
      `INSERT INTO gpt_accounts (email, token, refresh_token, user_count, max_capacity, reserved_slots, chatgpt_account_id, oai_device_id, expire_at, is_open, is_banned, remark, client_profile_key, client_user_agent, client_accept_language, client_oai_language, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, DATETIME('now', 'localtime'), DATETIME('now', 'localtime'))`,
      [
        normalizedEmail,
        token,
        refreshToken || null,
        ...
        generatedClientProfile.clientOaiLanguage
		    ...
		    `)
    const account = mapAccountRow(accountResult[0].values[0])
    saveDatabase()
  ...
  try {
    ...
    saveDatabase()
		    ...
		    const result = db.exec(`
		      ...
		      WHERE id = ?
		    `, [req.params.id])
    const account = mapAccountRow(result[0].values[0])
...
router.post('/:id/refresh-token', async (req, res) => {
  try {
    const db = await getDatabase()
	    ...
	    const result = db.exec(
	      `SELECT ${ACCOUNT_SELECT_FIELDS} FROM gpt_accounts WHERE id = ?`,
	      [req.params.id]
	    )
    ...
    const account = mapAccountRow(updatedResult[0].values[0])
