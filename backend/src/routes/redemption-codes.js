...
const mapCodeRow = (row, channelsByKey, options = {}) => {
  ...
  const status = row.length > 17 ? String(row[17] ?? '').trim().toLowerCase() || (row[2] === 1 ? 'used' : 'unused') : (row[2] === 1 ? 'used' : 'unused')
  const assignedAccountId = row.length > 18 ? toInt(row[18], 0) || null : null
  ...
  const processingStartedAtIndex = supplierBaseIndex + 9
  const lastErrorIndex = supplierBaseIndex + 10
  const prefixIndex = supplierBaseIndex + 11
  const batchNoIndex = supplierBaseIndex + 12
  return {
    id: row[0],
    code: row[1],
    isRedeemed: row[2] === 1,
    ...
    status,
    assignedAccountId,
    assignedAccountEmail: row[5] || null,
    activatedEmail,
    activatedAt,
    ...
    processingToken: row.length > processingTokenIndex ? row[processingTokenIndex] || null : null,
    processingStartedAt: row.length > processingStartedAtIndex ? row[processingStartedAtIndex] || null : null,
    lastError: row.length > lastErrorIndex ? row[lastErrorIndex] || null : null,
    prefix: row.length > prefixIndex ? row[prefixIndex] || null : null,
    batchNo: row.length > batchNoIndex ? row[batchNoIndex] || null : null
  }
}
...
const CODE_STATUS_PROCESSING = 'processing'
...
const CODE_SELECT_FIELDS = `
  id, code, is_redeemed, redeemed_at, redeemed_by,
  ...
  COALESCE(status, CASE WHEN is_redeemed = 1 THEN '${CODE_STATUS_USED}' ELSE '${CODE_STATUS_UNUSED}' END) AS status,
  assigned_account_id, activated_email, activated_at,
  COALESCE(is_downstream_sold, 0) AS is_downstream_sold, downstream_sold_at,
...
const resetStaleProcessingCode = (db, codeId) => {
  ...
  db.run(
    `
      UPDATE redemption_codes
      SET status = '${CODE_STATUS_UNUSED}',
          processing_token = NULL,
          processing_started_at = NULL,
          updated_at = DATETIME('now', 'localtime')
      WHERE id = ?
        AND COALESCE(status, '${CODE_STATUS_UNUSED}') = '${CODE_STATUS_PROCESSING}'
        AND is_redeemed = 0
    `,
    [codeId]
  )
...
const selectAccountCandidates = (db, limit = 100) => {
  const result = db.exec(
    `
      SELECT id,
             ...
             COALESCE(max_capacity, 5) AS max_capacity,
      ...
      ORDER BY
        CASE WHEN last_assigned_at IS NULL OR TRIM(last_assigned_at) = '' THEN 0 ELSE 1 END ASC,
...
const canUseAccountCandidate = (row, fallbackCapacity) => {
  if (!row) return false
  const token = String(row[2] ?? '').trim()
  const chatgptAccountId = String(row[7] ?? '').trim()
  if (!token || !chatgptAccountId) return false
  const expireAtMs = parseExpireAtToMs(row[13])
  if (expireAtMs == null || expireAtMs < Date.now()) return false
  const maxCapacity = Math.max(1, Number(row[5] || fallbackCapacity || 5))
  const used = Number(row[3] || 0) + Number(row[4] || 0) + Number(row[6] || 0)
  return used < maxCapacity
}

const reserveAccountSeat = (db, accountId) => {
  if (!db || !accountId) return false
  db.run(
    `
      UPDATE gpt_accounts
      SET reserved_slots = COALESCE(reserved_slots, 0) + 1,
          last_assigned_at = DATETIME('now', 'localtime'),
          updated_at = DATETIME('now', 'localtime')
      WHERE id = ?
  ...
  return typeof db.getRowsModified === 'function' ? db.getRowsModified() > 0 : true
}

const releaseAccountSeat = (db, accountId) => {
  if (!db || !accountId) return
  db.run(
    `
      UPDATE gpt_accounts
      SET reserved_slots = CASE
            WHEN COALESCE(reserved_slots, 0) > 0 THEN reserved_slots - 1
            ELSE 0
          END,
          updated_at = DATETIME('now', 'localtime')
      WHERE id = ?
...
const allocateAvailableAccountForRedemption = (db, fallbackCapacity) => {
  const candidates = selectAccountCandidates(db, 100)
  for (const candidate of candidates) {
    if (!canUseAccountCandidate(candidate, fallbackCapacity)) continue
    const accountId = Number(candidate[0] || 0)
    if (!accountId) continue
    if (!reserveAccountSeat(db, accountId)) continue
    return {
      accountId,
      email: candidate[1],
      token: candidate[2],
      userCount: Number(candidate[3] || 0),
      inviteCount: Number(candidate[4] || 0),
      maxCapacity: Math.max(1, Number(candidate[5] || fallbackCapacity || 5)),
      reservedSlots: Number(candidate[6] || 0) + 1,
      chatgptAccountId: candidate[7],
      oaiDeviceId: candidate[8],
      clientProfileKey: candidate[9],
      clientUserAgent: candidate[10],
      clientAcceptLanguage: candidate[11],
      clientOaiLanguage: candidate[12],
      expireAt: candidate[13]
    }
  }
  return null
}
...
export async function redeemCodeInternal({
  ...
  const currentCodeStatus = String(codeRecord.status || '').trim().toLowerCase()
  if (currentCodeStatus === CODE_STATUS_INVALID) {
    throw new RedemptionError(400, '该兑换码已失效，请联系管理员')
  }
  if (currentCodeStatus === CODE_STATUS_PROCESSING) {
    if (isCodeProcessingExpired(codeRecord.processingStartedAt)) {
      resetStaleProcessingCode(db, codeId)
      saveDatabase()
    } else {
      throw new RedemptionError(409, '该兑换码正在处理中，请稍后重试')
    }
  }

  const dynamicProcessingToken = `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`
  db.run(
    `
      UPDATE redemption_codes
      SET status = '${CODE_STATUS_PROCESSING}',
          processing_token = ?,
      ...
      WHERE id = ?
        AND is_redeemed = 0
    `,
    [dynamicProcessingToken, codeId]
  )
  saveDatabase()

  const requestedCapacityLimit = Math.max(1, Number(capacityLimit) || 5)
  const allocationErrors = []
  for (let allocationAttempt = 0; allocationAttempt < 100; allocationAttempt += 1) {
    const allocatedAccount = allocateAvailableAccountForRedemption(db, requestedCapacityLimit)
    if (!allocatedAccount) break

    const allocationAccountData = {
      token: allocatedAccount.token,
      chatgpt_account_id: allocatedAccount.chatgptAccountId,
      oai_device_id: allocatedAccount.oaiDeviceId,
      client_profile_key: allocatedAccount.clientProfileKey,
      client_user_agent: allocatedAccount.clientUserAgent,
      client_accept_language: allocatedAccount.clientAcceptLanguage,
      client_oai_language: allocatedAccount.clientOaiLanguage
    }

    let allocationInviteResult = { success: false, error: 'invite_failed' }
    let allocationSyncedAccount = null
    let allocationSyncedUserCount = null
    let allocationSyncedInviteCount = null

    try {
      allocationInviteResult = await inviteUserToChatGPTTeam(normalizedEmail, allocationAccountData, { proxyKey: allocatedAccount.accountId })

      if (!allocationInviteResult.success) {
        allocationErrors.push(String(allocationInviteResult.error || `invite_failed:${allocatedAccount.email}`))
        releaseAccountSeat(db, allocatedAccount.accountId)
        saveDatabase()
        continue
      }

      try {
        const userSync = await syncAccountUserCount(allocatedAccount.accountId, {
          userListParams: { offset: 0, limit: 1, query: '' }
        })
        allocationSyncedAccount = userSync.account
        if (typeof userSync.syncedUserCount === 'number') {
          allocationSyncedUserCount = userSync.syncedUserCount
        }
      } catch (syncError) {
        console.warn('同步账号人数失败:', syncError)
      }

      try {
        const inviteSync = await syncAccountInviteCount(allocatedAccount.accountId, {
          inviteListParams: { offset: 0, limit: 1, query: '' }
        })
        allocationSyncedAccount = inviteSync.account || allocationSyncedAccount
        if (typeof inviteSync.inviteCount === 'number') {
          allocationSyncedInviteCount = inviteSync.inviteCount
        }
      } catch (syncError) {
        console.warn('同步邀请数量失败:', syncError)
      }
      ...
      const updateParams = [
        redeemerIdentifier,
        normalizedEmail,
        allocatedAccount.email,
        allocatedAccount.accountId,
        resolvedOrderType
      ]
      ...
      const updatedRows = typeof db.getRowsModified === 'function' ? db.getRowsModified() : 1
      if (updatedRows === 0) {
        throw new RedemptionError(400, '该兑换码已被使用')
      }
      ...
      const resolvedUserCount = typeof allocationSyncedAccount?.userCount === 'number'
        ? allocationSyncedAccount.userCount
        : typeof allocationSyncedUserCount === 'number'
          ? allocationSyncedUserCount
          : allocatedAccount.userCount
      const resolvedInviteCount = typeof allocationSyncedAccount?.inviteCount === 'number'
        ? allocationSyncedAccount.inviteCount
        : typeof allocationSyncedInviteCount === 'number'
          ? allocationSyncedInviteCount
          : null

      return {
        data: {
          accountEmail: allocatedAccount.email,
        ...
        metadata: {
          ...
          requestedChannel,
          accountEmail: allocatedAccount.email,
          accountId: allocatedAccount.accountId
        }
      }
    } catch (allocationError) {
      releaseAccountSeat(db, allocatedAccount.accountId)
      saveDatabase()
      if (allocationError instanceof RedemptionError && allocationError.statusCode < 500) {
        throw allocationError
      }
      allocationErrors.push(String(allocationError?.message || `allocation_failed:${allocatedAccount.email}`))
    }
  }
  ...
  db.run(
    `
      ...
      WHERE id = ?
        AND is_redeemed = 0
    `,
    [allocationErrors.join(' | ').slice(0, 1000) || 'no_available_account', codeId]
  )
  saveDatabase()
  throw new RedemptionError(503, '暂无可用账号，请稍后再试')
  ...
  try {
    ...
    const requestedBatchNo = String(req.body?.batchNo ?? req.body?.batch_no ?? '').trim() || null
    {
      ...
      for (let i = 0; i < requestedCount; i++) {
        let code = generateRedemptionCode(12, requestedPrefix)
        let attempts = 0
        let success = false
          ...
          try {
            db.run(
              `INSERT INTO redemption_codes (
                code, channel, channel_name, status, prefix, batch_no, created_at, updated_at
              ) VALUES (?, ?, ?, ?, ?, ?, DATETIME('now', 'localtime'), DATETIME('now', 'localtime'))`,
              [code, normalizedChannel, resolvedChannelName, CODE_STATUS_UNUSED, requestedPrefix || null, requestedBatchNo]
            )
            createdCodes.push(code)
            success = true
          } catch (err) {
            if (String(err?.message || '').includes('UNIQUE')) {
              code = generateRedemptionCode(12, requestedPrefix)
              attempts++
            } else {
              throw err
            }
        ...
        if (!success) {
          failedCodes.push(`duplicate_after_${attempts}_attempts`)
        }
      }

      saveDatabase()
      const result = createdCodes.length > 0
        ? db.exec(`
            SELECT ${CODE_SELECT_FIELDS}
            FROM redemption_codes
      ...
      return res.status(201).json({
        ...
        prefix: requestedPrefix || null,
        batchNo: requestedBatchNo,
        generatedCount: createdCodes.length
      })
    }
...
router.delete('/:id', authenticateToken, requireMenu('redemption_codes'), async (req, res) => {
  try {
    ...
    const currentCode = checkResult[0].values[0]
    if (Number(currentCode[1] || 0) === 1 || String(currentCode[2] || '').trim().toLowerCase() === CODE_STATUS_PROCESSING) {
      return res.status(400).json({ error: '仅允许删除未使用且未处理中卡密' })
    }
...
router.post('/batch-delete', authenticateToken, requireMenu('redemption_codes'), async (req, res) => {
  try {
    ...
    const placeholders = ids.map(() => '?').join(',')
    {
      ...
      const deletedCount = typeof db.getRowsModified === 'function' ? db.getRowsModified() : ids.length
      return res.json({ message: `成功删除 ${deletedCount} 个兑换码` })
    }
