...
const ensureCoreIndexes = (database) => {
  ...
  changed = ensureIndex(
    database,
    'idx_gpt_accounts_capacity_state',
    'CREATE INDEX IF NOT EXISTS idx_gpt_accounts_capacity_state ON gpt_accounts (is_open, is_banned, max_capacity, user_count, invite_count, reserved_slots)'
  ) || changed

  changed = ensureIndex(
    database,
    'idx_gpt_accounts_last_assigned_at',
    'CREATE INDEX IF NOT EXISTS idx_gpt_accounts_last_assigned_at ON gpt_accounts (last_assigned_at, id)'
  ) || changed
  ...
  changed = ensureIndex(
    database,
    'idx_redemption_codes_status_created_at',
    'CREATE INDEX IF NOT EXISTS idx_redemption_codes_status_created_at ON redemption_codes (status, created_at)'
  ) || changed

  changed = ensureIndex(
    database,
    'idx_redemption_codes_processing_token',
    'CREATE INDEX IF NOT EXISTS idx_redemption_codes_processing_token ON redemption_codes (processing_token)'
  ) || changed

  changed = ensureIndex(
    database,
    'idx_redemption_codes_batch_no',
    'CREATE INDEX IF NOT EXISTS idx_redemption_codes_batch_no ON redemption_codes (batch_no, created_at)'
  ) || changed
...
export async function initDatabase() {
  ...
  try {
    const gptAccountColumns = getTableColumns(database, 'gpt_accounts')
    if (gptAccountColumns.size > 0) {
      if (!gptAccountColumns.has('max_capacity')) {
        database.run('ALTER TABLE gpt_accounts ADD COLUMN max_capacity INTEGER DEFAULT 5')
      }
      if (!gptAccountColumns.has('reserved_slots')) {
        database.run('ALTER TABLE gpt_accounts ADD COLUMN reserved_slots INTEGER DEFAULT 0')
      }
      if (!gptAccountColumns.has('last_assigned_at')) {
        database.run('ALTER TABLE gpt_accounts ADD COLUMN last_assigned_at DATETIME')
      }
      database.run(
        `UPDATE gpt_accounts
         SET max_capacity = 5
    ...
    const redemptionColumns = getTableColumns(database, 'redemption_codes')
    if (redemptionColumns.size > 0) {
      const missingRedemptionColumns = [
        ['status', "ALTER TABLE redemption_codes ADD COLUMN status TEXT DEFAULT 'unused'"],
        ['assigned_account_id', 'ALTER TABLE redemption_codes ADD COLUMN assigned_account_id INTEGER'],
        ['activated_email', 'ALTER TABLE redemption_codes ADD COLUMN activated_email TEXT'],
        ['activated_at', 'ALTER TABLE redemption_codes ADD COLUMN activated_at DATETIME'],
        ['processing_token', 'ALTER TABLE redemption_codes ADD COLUMN processing_token TEXT'],
        ['processing_started_at', 'ALTER TABLE redemption_codes ADD COLUMN processing_started_at DATETIME'],
        ['last_error', 'ALTER TABLE redemption_codes ADD COLUMN last_error TEXT'],
        ['prefix', 'ALTER TABLE redemption_codes ADD COLUMN prefix TEXT'],
        ['batch_no', 'ALTER TABLE redemption_codes ADD COLUMN batch_no TEXT'],
      ]

      for (const [name, ddl] of missingRedemptionColumns) {
        if (!redemptionColumns.has(name)) {
          database.run(ddl)
        }
      }
      ...
      database.run(
        `UPDATE redemption_codes
         SET status = CASE
           WHEN is_redeemed = 1 THEN 'used'
           WHEN status IS NULL OR trim(status) = '' THEN 'unused'
           ELSE status
         END`
      )
      database.run(
        `UPDATE redemption_codes
         ...
         WHERE is_redeemed = 1
           AND (activated_email IS NULL OR activated_email = '')`
      )
      database.run(
        `UPDATE redemption_codes
    ...
    console.log('[DB] extended migration skipped:', err.message)
  }
